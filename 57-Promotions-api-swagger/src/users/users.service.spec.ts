...
describe('Сервис пользователей', () => {
	...
	describe('Создание поставщика', () => {
		it('Должен создать поставщика с хэшированным паролем', async () => {
			...
			const result = await usersService.createSupplier({
				email: 'test@example.com',
				name: 'Test User',
				password: 'password123',
			});
	...
	describe('Удаление поставщика', () => {
		...
		it('Должен выбросить HTTPError, если у поставщика есть активные акции', async () => {
			usersRepository.findById = jest.fn().mockResolvedValue(mockUser);
			PrismaServiceMock.client.promotionModel.findMany = jest
				.fn()
				.mockResolvedValue([{ id: 1, endDate: new Date('2025-12-31') }]);
