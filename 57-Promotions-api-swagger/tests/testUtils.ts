...
export const cleanUp = async (prisma: PrismaClient) => {
  const deletedPromotions = await prisma.promotionModel.deleteMany({});
  const deletedUsers = await prisma.userModel.deleteMany({
    where: {
      OR: [
  ...
  console.log(`Deleted ${deletedPromotions.count} promotions and ${deletedUsers.count} users`);
};
