...
import { UserWithAddressAndCategories } from '../users/users.repository.interface';
...
export class TelegramBotController implements ITelegramBotController {
	...
	private async setupMiddlewares(): Promise<void> {
		...
		this.bot.on('text', async (ctx: ExtendedContext, next) => {
			if (
				ctx.session.awaitingAddressInput &&
				ctx.message &&
	...
	private async sendResponse(
		...
		if (keyboard) {
			...
			} else if (!editMessage && !isCallback) {
				replyOptions = { reply_markup: keyboard };
			}
		}
		try {
